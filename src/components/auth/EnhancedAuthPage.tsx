...
export const EnhancedAuthPage: React.FC<{ onAuthSuccess: () => void }> = ({ onAuthSuccess }) => {
  ...
  const validatePasswordRealTime = (password: string, confirmPassword: string = signupData.confirmPassword) => {
    const validation: PasswordValidation = {
      length: password.length >= 6 && password.length <= 20, // Updated to 6-20 characters
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSignupData(prev => ({ ...prev, idProofFile: file }));
  ...
  const handleIdProofUpload = async (file: File) => {
    try {
      ...
      const { data, error } = await supabase.storage
        .from('documents')
        .upload(`id-proofs/${fileName}`, file);
    ...
    try {
      ...
      try {
        ...
        if (!authError && authUsers?.users) {
          const emailExists = authUsers.users.some((user: { email?: string }) => user.email === signupData.email);
          if (emailExists) {
            toast({
      ...
      toast({
        ...
        description: signupData.role === 'provider' 
          ? "Your provider account is pending approval. You'll be notified once approved."
              ...
              <form onSubmit={handleSignup} className="space-y-5">
                          ...
                          {signupData.idProofFile ? (
                            <div className="space-y-2 animate-in zoom-in-50 duration-500">
                              <div className="w-12 h-12 bg-gradient-to-r from-green-100 to-emerald-100 rounded-lg flex items-center justify-center mx-auto animate-bounce">
                                {signupData.idProofFile.type.startsWith('image/') ? (
                    ...
                    <div className="bg-gradient-to-r from-gray-50 to-purple-50 rounded-lg p-4 space-y-3 animate-in slide-in-from-top-2 duration-300">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        <div className={`flex items-center space-x-2 transition-all duration-300 ${
                          passwordValidation.length ? 'text-green-600 scale-105' : 'text-gray-400'
                        }`}>
                        ...
                        <div className={`flex items-center space-x-2 transition-all duration-300 ${
                          passwordValidation.uppercase ? 'text-green-600 scale-105' : 'text-gray-400'
                        }`}>
                        ...
                        <div className={`flex items-center space-x-2 transition-all duration-300 ${
                          passwordValidation.lowercase ? 'text-green-600 scale-105' : 'text-gray-400'
                        }`}>
                        ...
                        <div className={`flex items-center space-x-2 transition-all duration-300 ${
                          passwordValidation.number ? 'text-green-600 scale-105' : 'text-gray-400'
                        }`}>
                      ...
                      <div className="mt-3">
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className={`font-medium ${
                            ...
                            [passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 4 ? 'text-green-600' :
                            [passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 2 ? 'text-yellow-600' : 'text-red-500'
                          }`}>
                            {[passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 4 ? 'Strong' :
                             [passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 2 ? 'Medium' : 'Weak'}
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className={`h-2 rounded-full transition-all duration-500 ${
                              // Count only the 4 main requirements (exclude match)
                              [passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 4 ? 'bg-green-500 w-full' :
                              [passwordValidation.length, passwordValidation.uppercase, passwordValidation.lowercase, passwordValidation.number].filter(Boolean).length >= 2 ? 'bg-yellow-500 w-2/3' : 'bg-red-500 w-1/3'
                            }`}
                          />
                        </div>
                      </div>
                ...
                <div className="space-y-2">
                  ...
                  {signupData.confirmPassword && (
                    <div className={`flex items-center space-x-2 text-xs transition-all duration-300 animate-in slide-in-from-left-2 ${
                      signupData.password === signupData.confirmPassword ? 'text-green-600 scale-105' : 'text-red-500'
                    }`}>
                      <div className={`transition-all duration-300 ${
                        signupData.password === signupData.confirmPassword ? 'animate-bounce' : 'animate-pulse'
                      }`}>
